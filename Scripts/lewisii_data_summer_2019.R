##################################################Density and Distance from 2016 from averages######################################
...
model_density_distance <- lmer(log(Number_of_Plants) ~ distance_from_2016 + (1|Foreland_Code.x), data=density_df2)
...
model_seedlings_distance <- glmer(number_of_seedlings ~ distance_from_2016 + (1|Foreland_Code), data=prop_flowering)
...
####make histogram w/seedlings and distance from terminus
subset_seedlings <- subset(lewisii_data,seedling==1,select=dist)
hist(as.numeric(subset_seedlings))
